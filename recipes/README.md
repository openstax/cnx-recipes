...
## Unit
`$Config_PartType_Unit_TitleContent` contains the formatting for Unit title. See [TitleContent](#titlecontent).
...
In some books (anatomy) the Preface Abstract is discarded.
...
- if `outlineTitle` is truthy _and_ `hasLearningObjectives` is true then the abstract is wrapped in a `<div class="learning-objective">` element

An autogenerated chapter outline is added (old books use this). Newer books then remove this.


## Section
`$Config_PartType_Section_TitleContent` contains the formatting for an Section title. See [TitleContent](#titlecontent).
Usually the formatting ends up being `<.os-number>1.6</>`.

Section titles are converted from an `<h2 ...>` to a `<div ...>`.

The section itself gets a `.chapter-content-module` class added to it.
...
Wrap the entire table inside a `div.os-table` (for styling and REX depends on this for search) and inject a `.os-table-title` child.

Generate a summary attribute using the table's caption and onto the newly-created `div.os-table` wrapper.

The title is (???? where does the title come from?)

```html
<div class="os-table" summary="this is in the table caption">
  <div class="os-table-title">...Table 1.4...</div>

  <!-- Original table goes here ... -->
</div>
```
